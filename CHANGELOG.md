...
- 修复：最小化后不聚焦下一个窗口。最小化与关闭在系统里表现完全不同：关闭会让窗口从 AX 窗口列表消失（~281ms），而最小化会**保留**该条目并打上「已最小化」标记；此前以「AX 列表里没有该窗口」作为判定条件，对最小化永远不成立。同时实测发现最小化动画期间目标应用的 AX 服务会停止响应，读取窗口列表会阻塞约 515ms（关闭时不会）。因此最小化改为只做轻量的「屏幕列表」检查，并在此后 100ms 内信任用户的明确操作；关闭与隐藏仍严格要求验证。实测最小化约 118ms
