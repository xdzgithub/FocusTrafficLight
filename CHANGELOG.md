...
- 优化：缩短「关闭后聚焦」的等待。去掉触发后 30ms 的沉降延迟、轮询间隔 25ms→15ms，并把「关闭/隐藏」的等待上限从 800ms 收到 350ms（最小化仍保留 800ms，因为它必须等 genie 动画结束）。实测：Finder 84–115ms，系统设置 129ms，Chrome 110ms（此前约 290–320ms）。关标签页这类窗口仍在的场景依旧正确跳过、不抢焦点
