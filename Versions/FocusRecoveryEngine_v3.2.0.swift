...
final class FocusRecoveryEngine {
    ...
    func performRecoveryCheck() {
        ...
        if isFinderQuickLookActive() {
            print("[FocusTrafficLight] Guard 1: Finder Quick Look active, skipping recovery")
            return
        }
        ...
        if let frontmostApp = NSWorkspace.shared.frontmostApplication,
           frontmostAppHasVisibleWindow(frontmostApp) {
            print("[FocusTrafficLight] Guard 2: Frontmost app has visible window, skipping recovery")
            return
        }
        ...
        if let focusedWindow = getCurrentFocusedWindow() {
            if isUsableFocusedWindow(focusedWindow) {
                print("[FocusTrafficLight] Guard 3: Usable focused window exists, skipping recovery")
                return
            }
        }

        // Guard 4: Find and focus the topmost valid recovery target
        print("[FocusTrafficLight] All guards cleared — performing focus recovery")
        if let (window, app) = findTopmostValidWindow() {
            ...
            print("[FocusTrafficLight] No valid recovery target found")
        }
    }
