...
final class WindowManager {
    ...
    private func doFocusCheck() {
        ...
        // Check if current focused window is already valid
        let currentWindow = getCurrentFocusedWindow()
        if let window = currentWindow, hasTrafficLights(window), !isMinimized(window) {
            print("[FocusTrafficLight] Current window valid, done")
            return
        }

        // Find topmost valid window from Z-order
        if let (window, app) = findTopmostWindowWithCGWindowList() {
    ...
    private func findTopmostWindowWithCGWindowList() -> (AXUIElement, NSRunningApplication)? {
        // CGWindowListCopyWindowInfo returns windows in Z-order (front to back)
        // kCGNullWindowID means all windows, .optionOnScreenOnly for only visible windows
        ...
        for windowInfo in windowList {
            ...
            // Get window bounds
            guard let bounds = windowInfo[kCGWindowBounds as String] as? [String: CGFloat] else { continue }
            ...
            guard let axWindow = getFirstAXWindow(for: app) else { continue }

            // Check if valid (has traffic lights, not minimized)
            guard hasTrafficLights(axWindow), !isMinimized(axWindow) else { continue }
    ...
    private func getFirstAXWindow(for app: NSRunningApplication) -> AXUIElement? {
        ...
        guard AXUIElementCopyAttributeValue(appElement, kAXWindowsAttribute as CFString, &windows) == .success,
              ...
              let firstWindow = windowList.first else {
            return nil
        }
